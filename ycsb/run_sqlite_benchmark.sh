...
# Script to run SQLite benchmarks using go-ycsb
...
RESULT_FILE="${OUTPUT_DIR}/sqlite_results.json"
...
# Test database files (SQLite is a single-file database)
SWORNDISK_FILE="${SWORNDISK_DATA_ROOT}/sqlite.db"
CRYPTDISK_FILE="${CRYPTDISK_DATA_ROOT}/sqlite.db"
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}SQLite Benchmark - go-ycsb${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
mkdir -p "${OUTPUT_DIR}"
echo "{" > "${RESULT_FILE}"
echo "  \"benchmark\": \"SQLite\"," >> "${RESULT_FILE}"
...
run_benchmark() {
    ...
    "${YCSB_BIN}" load sqlite -P "${WORKLOAD_DIR}/${workload}" -p sqlite.db="${db_file}"
    ...
    local output=$(mktemp)
    "${YCSB_BIN}" run sqlite -P "${WORKLOAD_DIR}/${workload}" -p sqlite.db="${db_file}" 2>&1 | tee "${output}"
...
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}All SQLite benchmarks completed!${NC}"
echo -e "${GREEN}========================================${NC}"
echo ""
